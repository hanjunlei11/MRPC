...
def feed_forward(inputs,seq_len=None,keep_rate=None,activition='relu'):
    ...
    dense = Dense(inputs=inputs,seq_len=seq_len, ouput_size=400,keep_rate=keep_rate,activition=activition)
...
def encoder(inputs,embedding_size,nb_layers,nb_head,size_per_head,Q_len=None,V_len=None,training=True,keep_rate=None,activition='relu',use_position = False):
    ...
    for i in range(nb_layers):
        ...
        dense_1 = Dense(inputs=output,ouput_size=embedding_size,keep_rate=keep_rate,seq_len=Q_len,activition=activition)
        dense_1 = tf.layers.batch_normalization(dense_1,training=training)
        batch = dense_1
    return batch
